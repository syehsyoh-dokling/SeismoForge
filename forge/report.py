...
def render_report(
    ...
    for check in acceptance["checks"]:
        if check["check"] == "all_records_converged":
            ...
            continue
        status = "OK" if check["satisfied"] else "**FAIL**"
        add(f"| {check['check']} | {check['value']:.4g} | {check['limit']:.4g} "
            f"| {check['utilization']:.2f} | {status} |")
    add("")
    if acceptance["governing_check"]:
        add(f"Governing check: **{acceptance['governing_check']}** at "
            f"utilization {acceptance['governing_utilization']:.2f}.")
    add("")
    ...
    for record in assessment["per_record"]:
        residual = record["residual_disp_m"]
        add(
            f"| {record['record_id']} | {record['seed']} | "
            f"{'yes' if record['converged'] else 'NO'} | "
            f"{record['peak_isolator_disp_m']:.4f} | "
            f"{record['peak_drift_ratio']:.5f} | "
            f"{record['peak_floor_accel_g']:.4f} | "
            f"{record['base_shear_coeff']:.4f} | "
            f"{residual if residual is None else f'{residual:.4f}'} |"
        )
    add("")
    ...
    add("---")
    add("*Generated by SeismoForge. Every number above comes from the "
        "simulation suite described in the verification basis; the suite is "
        "deterministic and reproducible from the project brief alone.*")
    add("")
    return "\n".join(lines)
